...
os.system("clear")
autoketik(f"{biru}[{kuning}Warning{biru}] {W}Welcome To My Script")
time.sleep(3)
os.system("Good Happy For This")
time.sleep(3)
os.system("clear")
autoketik(f"""
...
{abu}-----------------------------------------
{putih}[{biru}•{putih}] {biru}Author {putih}   : Ucok_Hunter
{putih}[{biru}•{putih}] {abu}GitHub {putih}   : Ucok_Cyber_Muslim
{W}[{Y}•{W}] You IP {putih}  :{Y} {ip}
{W}[{Y}•{W}] Time {putih}:{Y} {localtime}
{W}[{Y}•{W}] Total Run {putih}:{Y} {getvisit['value']}
""")

nomor = input(f"{W}[{R}• {kuning}•{hijau}•{W}] {biru}Number {W}: ")
jum = int(input(f"{W}[{R}• {kuning}•{hijau}•{W}] {biru}How ? {W}: "))
for i in range(jum):
